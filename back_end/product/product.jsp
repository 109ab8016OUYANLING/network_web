...
<%
		...
		String input_img = request.getParameter("a2");
		String input_name = request.getParameter("a3");
		String input_price = request.getParameter("a4");
		String input_num = request.getParameter("a5");
		...
		if(con.isClosed())
			...
			if( input_img == null ){
				...
				sql="UPDATE product SET img= '"+input_img+"',name = '"+input_name+"' ,price = '"+input_price+"',num = '"+input_num+"',YN = '"+cb_YN+"' WHERE No = '"+No+"'";
			...
			if ( newp!=null ){
				...
				rs.last();
				np=rs.getRow()+1;
				sql = "INSERT product(No,img,name,price,num,intro,YN) " +
                  "VALUES ('"+np+"','"+psrc+"','"+pname+"','"+pprice+"','"+pnum+"','"+pcontent+"','Y')";
				con.createStatement().execute(sql); //執行成功傳回false
 
			}
			
			  
			sql = "SELECT No,img,name,price,num,YN FROM product";
...
<%			
			while(tmp.next())
			{
				...
				if(checked.equals("Y")){
				out.print("<td><input type='checkbox' name='YN' value='"+n+"' checked></td>"); 
				}
				else{
				out.print("<td><input type='checkbox' name='YN' value='"+n+"'></td>"); 
				}
				out.print("</form>");
				out.print("</tr>");
				}
